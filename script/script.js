document.addEventListener("DOMContentLoaded", () => {
    ...
    // إنشاء الفيديوهات ديناميكياً بشكل تدريجي مع ترتيب الجديد بالأعلى والقديم بالأسفل
    const videosContainer = document.getElementById("videosContainer");
    let videoIds = Object.keys(videosData);
    videoIds = videoIds.reverse(); // عكس الترتيب لجعل الجديد بالأعلى والقديم بالأسفل
    let currentIndex = 0;
    ...
    function showNoResultsMessage() {
        searchResults.innerHTML = '<ssw>Here the search results will appear.</ssw>';
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function applyArabicFont() {
    ...
    if (html.lang === "ar") {
      const fontFamily =
        '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Oxygen, Oxygen-Sans, Ubuntu, Cantarell, "Helvetica Neue", "Open Sans", Arial, sans-serif';

      body.style.fontSize = "13px";
      body.style.fontFamily = fontFamily;

      // تطبيق نفس الخط على جميع عناصر الإدخال
      inputs.forEach((el) => {
        el.style.fontSize = "13px";
        el.style.fontFamily = fontFamily;
      });
    } else {
      // إعادة القيم الأصلية عند تغيير اللغة لغير العربية
      body.style.fontSize = "";
